...
public class GigaflopEstimator {
	...
	// This measurement can be unprecise. See: http://elastic-project.com/gigaflop_estimation
	...
	public static int get_repetitions(long firstTime){
		if(firstTime==0) return 25; // arbitrary
		
		// We do not want to measure longer than 0.1 seconds but at least one repetition
	...
	public static void measure_baseline(){
		if(!has_already_measured_baseline() && (!failed || last_failed < System.currentTimeMillis()-10*1000)){
			try {
				...
				Logger.logInfoMessage("We measured, that the simplest program executed in " + baseline_duration_mseconds + " MILLISECONDS on this machine ...");
			} catch (ParseException e) {
				failed = true;
				last_failed = System.currentTimeMillis();
				baseline_duration_mseconds = 0;
				Logger.logErrorMessage("could not evaluate the simple program in the gigaflop estimator! Retrying soon ...");
			}
		}
	}
	...
	public static void measure_and_store_source(long workid, byte[] src){
		measure_baseline();
		if(!has_already_measured_baseline()) return;
		
		try {
			double dur = measure_source(src, false);
			
			if(dur == 0) return; // Impossible, ignore this crap
			
			Logger.logInfoMessage("We measured, that work_id " + workid + " can be executed in " + baseline_duration_mseconds + " MILLISECONDS on this machine ...");
			durations_map.put(workid, dur);
			
			// Update baseline if neccessary, should not happen but hey ... doesn't hurt
	...
	private static double measure_source(byte[] src, boolean debug) throws ParseException {
		...
		Executioner.executeProofOfWork(src,personalizedIntStreamPseudo(Genesis.CREATOR_PUBLIC_KEY, 12345678, 87654321), Constants.least_possible_target, Constants.least_possible_target);
		long estimatedTime = System.currentTimeMillis() - startTime;
		if(debug)
			Logger.logInfoMessage("First measurement: " + estimatedTime + " MILLISECONDS on this machine ...");

		// Be more precise on fast systems, if it took less than 0.1 second, repeat the measurement to fill that 0.1s
		if(estimatedTime < 100){
			int rept = get_repetitions(estimatedTime);
			startTime = System.currentTimeMillis();    
			for(int i=1;i<=rept;++i){
				Executioner.executeProofOfWork(src,personalizedIntStreamPseudo(Genesis.CREATOR_PUBLIC_KEY, 12345678+i, 87654321+i), Constants.least_possible_target, Constants.least_possible_target);
			}
			estimatedTime = (System.currentTimeMillis() - startTime);
			if(debug)
				Logger.logInfoMessage("Second measurement (" + rept + " repetitions): " + estimatedTime + " MILLISECONDS on this machine ...");
			...
			if(estimatedTime < 100){
				...
				estimatedTime = (System.currentTimeMillis() - startTime);
				if(debug)
					Logger.logInfoMessage("Third measurement (" + rept + " repetitions): " + estimatedTime + " MILLISECONDS on this machine ...");
	...
	public static int[] personalizedIntStreamPseudo(byte[] publicKey, long blockId, long workId){
    	...
    	dig.reset();
    	dig.update(publicKey); // do public key instead of multiplicator! Just to have the same amount of "hashing overhead".
