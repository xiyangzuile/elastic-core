...
public abstract class VersionedEntityDbTable<T> extends EntityDbTable<T> {
	...
	protected VersionedEntityDbTable(final String table, final DbKey.Factory<T> dbKeyFactory, final String fullTextSearchColumns) {
	...
	public final boolean delete(final T t, final boolean keepInCache) {
		...
		try (Connection con = DerivedDbTable.db.getConnection();
				PreparedStatement pstmtCount = con.prepareStatement("SELECT 1 FROM " + this.table
						+ this.dbKeyFactory.getPKClause() + " AND height < ? LIMIT 1")) {
