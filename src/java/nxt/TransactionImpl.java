...
final class TransactionImpl implements Transaction {
	...
	public long getId() {
		if (id == 0) {
			...
			BigInteger bigInteger = new BigInteger(1, new byte[] { fullHash[7], fullHash[6], fullHash[5], fullHash[4],
					fullHash[3], fullHash[2], fullHash[1], fullHash[0] });
			...
			try {
				...
				if (signature != null) {
					Logger.logDebugMessage(
							"Failed to get transaction bytes for transaction: " + getJSONObject().toJSONString());
				}
				throw e;
			}
		}
	...
	static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments)
		...
		if (prunableAttachments != null) {
			...
			Appendix.PrunableEncryptedMessage prunableEncryptedMessage = Appendix.PrunableEncryptedMessage
					.parse(prunableAttachments);
			if (prunableEncryptedMessage != null) {
	...
	static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData)
			throws NxtException.NotValidException {
		try {
	...
	public void validate() throws NxtException.ValidationException {
		...
		if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
			throw new NxtException.NotValidException(
					"Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
		}
		...
		if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
			throw new NxtException.NotValidException(
					"Transaction size " + getFullSize() + " exceeds maximum payload size");
		}
		...
		if (ecBlockId != 0) {
			if (blockchainHeight < ecBlockHeight) {
				throw new NxtException.NotCurrentlyValidException(
						"ecBlockHeight " + ecBlockHeight + " exceeds blockchain height " + blockchainHeight);
			}
			if (BlockDb.findBlockIdAtHeight(ecBlockHeight) != ecBlockId) {
				throw new NxtException.NotCurrentlyValidException(
						"ecBlockHeight " + ecBlockHeight + " does not match ecBlockId "
								+ Long.toUnsignedString(ecBlockId) + ", transaction was generated on a fork");
			}
		}
	...
	void apply() {
		...
		if (referencedTransactionFullHash != null) {
			senderAccount.addToUnconfirmedBalanceNQT(getType().getLedgerEvent(), getId(), 0,
	...
	private long getMinimumFeeNQT(int blockchainHeight) {
		...
		for (Appendix.AbstractAppendix appendage : appendages) {
			...
			Fee fee = blockchainHeight >= appendage.getNextFeeHeight() ? appendage.getNextFee(this)
