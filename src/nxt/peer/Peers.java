...
public final class Peers {
	...
	private static final boolean cjdnsOnly;
	...
	static {
		...
		if (platform.length() > Peers.MAX_PLATFORM_LENGTH) platform = platform.substring(0, Peers.MAX_PLATFORM_LENGTH);
		myPlatform = platform;
		...
		if (Peers.myAddress != null) try {
            ...
            while (intfs.hasMoreElements()) {
                ...
                for (final InterfaceAddress intfAddr : intfAddrs) {
                    ...
                    for (final InetAddress myAddr : myAddrs)
                        if (Objects.equals(extAddr, myAddr)) {
                            addrValid = true;
                            break chkAddr;
                        }
                }
            ...
            if (!addrValid) {
                ...
                if (extAddr != null) if (Arrays.stream(myAddrs).anyMatch(myAddr -> Objects.equals(extAddr, myAddr)))
                    addrValid = true;
            }
            if (!addrValid) Logger.logWarningMessage("Your announced address does not match your external address");
		...
		if (Peers.myHallmark != null && Peers.myHallmark.length() > 0) try {
            final Hallmark hallmark = Hallmark.parseHallmark(Peers.myHallmark);
            if (!hallmark.isValid()) throw new RuntimeException("Hallmark is not valid");
            if (Peers.myAddress != null) {
                if (!Objects.equals(hallmark.getHost(), myHost))
                    throw new RuntimeException("Invalid hallmark host");
                if (myPort != hallmark.getPort()) throw new RuntimeException("Invalid hallmark port");
            }
		...
		if (Peers.myAddress != null) try {
            ...
            if (!Constants.isTestnet) if (port >= 0) announcedAddress = Peers.myAddress;
            else announcedAddress = host + (Peers.myPeerServerPort != Peers.DEFAULT_PEER_PORT
                        ? ":" + Peers.myPeerServerPort : "");
            else announcedAddress = host;
		...
		if (!Constants.ENABLE_PRUNING && Constants.INCLUDE_EXPIRED_PRUNABLE) servicesList.add(Peer.Service.PRUNABLE);
		...
		if (API.openAPIPort > 0 || API.openAPISSLPort > 0) {
			...
			API.disabledAPIs.forEach(apiName -> {
				...
				if (api != null) disabledAPISet.add(api);
			});
			...
			if (API.apiServerCORS) servicesList.add(Peer.Service.CORS);
		}

		long services = 0;
		for (final Peer.Service service : servicesList) services |= service.getCode();
		...
		if (knownBlacklistedPeersList.isEmpty()) knownBlacklistedPeers = Collections.emptySet();
        else knownBlacklistedPeers = Collections.unmodifiableSet(new HashSet<>(knownBlacklistedPeersList));
		...
		if (!Constants.isOffline) ThreadPool.runBeforeStart(new Runnable() {
		...
		ThreadPool.runAfterStart(() -> {
			for (final Future<String> unresolvedPeer : unresolvedPeers)
                try {
                    ...
                    if (badAddress != null) Logger.logDebugMessage("Failed to resolve peer address: " + badAddress);
	...
	private static final Runnable peerUnBlacklistingThread = () -> {
		...
		try {
			try {
				...
				for (final PeerImpl peer : Peers.peers.values()) peer.updateBlacklistedStatus(curTime);
			...
			try {
				...
				if (!Peers.hasEnoughConnectedPublicPeers(Peers.maxNumberOfConnectedPublicPeers)) {
					...
					if (!hallmarkedPeers.isEmpty() || !nonhallmarkedPeers.isEmpty()) {
						...
						for (int i = 0; i < 10; i++) {
							List<Peer> peerList;
							if (hallmarkedPeers.isEmpty()) peerList = nonhallmarkedPeers;
                            else if (nonhallmarkedPeers.isEmpty()) peerList = hallmarkedPeers;
                            else peerList = ThreadLocalRandom.current().nextInt(2) == 0 ? hallmarkedPeers
                                    : nonhallmarkedPeers;
							connectSet
				...
				Peers.peers.values().forEach(peer4 -> {
					if (peer4.getState() == Peer.State.CONNECTED && now - peer4.getLastUpdated() > 3600
							&& now - peer4.getLastConnectAttempt() > 600) Peers.peersService.submit(peer4::connect);
				...
				if (Peers.hasTooManyKnownPeers()
					...
					for (final PeerImpl peer5 : Peers.peers.values()) {
						if (now - peer5.getLastUpdated() > 24 * 3600) peer5.remove();
						if (Peers.hasTooFewKnownPeers()) break;
					}
					if (Peers.hasTooManyKnownPeers()) {
						final PriorityQueue<PeerImpl> sortedPeers = new PriorityQueue<>(Peers.peers.values());
						int skipped = 0;
						while (skipped < Peers.minNumberOfKnownPeers) {
							if (sortedPeers.poll() == null) break;
							skipped += 1;
						}
						while (!sortedPeers.isEmpty()) sortedPeers.poll().remove();
					}
				...
				for (final String wellKnownPeer : Peers.wellKnownPeers) {
					...
					if (peer6 != null && now - peer6.getLastUpdated() > 3600
							&& now - peer6.getLastConnectAttempt() > 600) Peers.peersService.submit(() -> {
                        Peers.addPeer(peer6);
	...
	private static final Runnable getMorePeersThread = new Runnable() {
		...
		public void run() {
			...
			try {
				try {
					if (Peers.hasTooManyKnownPeers()) return;
					...
					Peers.getAllPeers().forEach(myPeer -> {
						if (!myPeer.isBlacklisted() && myPeer.getAnnouncedAddress() != null
								...
								&& !Objects.equals(myPeer.getAnnouncedAddress(), peer.getAnnouncedAddress())) {
		...
		private void updateSavedPeers() {
			...
			Peers.peers.values().forEach(peer -> {
				if (peer.getAnnouncedAddress() != null && !peer.isBlacklisted()
						&& now - peer.getLastUpdated() < 7 * 24 * 3600) currentPeers.put(peer.getAnnouncedAddress(),
			...
			oldPeers.forEach(entry -> {
				if (currentPeers.get(entry.getAddress()) == null) toDelete.add(entry);
			});
			...
			//
			if (toDelete.isEmpty() && toUpdate.isEmpty()) return;
			//
	...
	static {
		Peers.addListener(peer -> Peers.peersService.submit(() -> {
			if (peer.getAnnouncedAddress() != null && !peer.isBlacklisted()) try {
                Db.db.beginTransaction();
	...
	static {
		if (!Constants.isOffline) {
			...
			if (Peers.getMorePeers) ThreadPool.scheduleThread("GetMorePeers", Peers.getMorePeersThread, 20);
		}
	}
	...
	static {
		...
		if (version.endsWith("e")) version = version.substring(0, version.length() - 1);
		final String[] versions = version.split("\\.");
		MAX_VERSION = new int[versions.length];
		for (int i = 0; i < versions.length; i++) Peers.MAX_VERSION[i] = Integer.parseInt(versions[i]);
	}
	...
	static PeerImpl findOrCreatePeer(final InetAddress inetAddress, final String announcedAddress,
		...
		if (Peers.cjdnsOnly && !Objects.equals(host.substring(0, 2), "fc")) return null;
		// re-add the [] to ipv6 addresses lost in getHostAddress() above
		if (host.split(":").length > 2) host = "[" + host + "]";

		PeerImpl peer;
		if ((peer = Peers.peers.get(host)) != null) return peer;
		if (!create) return null;

		if (Peers.myAddress != null && Peers.myAddress.equalsIgnoreCase(announcedAddress)) return null;
		if (announcedAddress != null && announcedAddress.length() > Peers.MAX_ANNOUNCED_ADDRESS_LENGTH) return null;
	...
	public static PeerImpl findOrCreatePeer(String announcedAddress, final boolean create) {
		...
		PeerImpl peer;
		if ((peer = Peers.peers.get(announcedAddress)) != null) return peer;
		String host = Peers.selfAnnouncedAddresses.get(announcedAddress);
		if (host != null && (peer = Peers.peers.get(host)) != null) return peer;
		try {
			...
			if (host2 != null && (peer = Peers.peers.get(host2)) != null) return peer;
			final InetAddress inetAddress = InetAddress.getByName(host);
	...
	private static List<Peer> getPeers(final Filter<Peer> filter, final int limit) {
	...
	private static List<Peer> getSnPeers(final Filter<Peer> filter) {
		return Peers.getSnPeers(filter, Integer.MAX_VALUE);
	}

	private static List<Peer> getSnPeers(final Filter<Peer> filter, final int limit) {
		final List<Peer> result = new ArrayList<>();
		for (final Peer peer : Peers.peers.values()) {
	...
	private static List<String> getPotentialSNPeersInternal(){
	...
	private static List<Peer> getConnectedSnPeers(){
	...
	private static boolean hasTooFewKnownPeers() {
	...
	public static boolean hasTooManyInboundPeers() {
		...
		return !Nxt.isSupernode && Peers.getPeers(Peer::isInbound, Peers.maxNumberOfInboundConnections).size() >= Peers.maxNumberOfInboundConnections;
	...
	private static boolean hasTooManyOutboundConnections() {
		return Peers
	...
	public static boolean isNewVersion(String version) {
		...
		for (int i = 0; i < Peers.MAX_VERSION.length && i < versions.length; i++)
            try {
                ...
                if (v > Peers.MAX_VERSION[i]) return true;
                else if (v < Peers.MAX_VERSION[i]) return false;
	...
	public static boolean isOldVersion(String version, final int[] minVersion) {
		...
		for (int i = 0; i < minVersion.length && i < versions.length; i++)
            try {
                ...
                else if (v < minVersion[i]) return true;
            } catch (final NumberFormatException e) {
                return true;
            }
	...
	public static PeerImpl removePeer(final Peer peer) {
		if (peer.getAnnouncedAddress() != null) Peers.selfAnnouncedAddresses.remove(peer.getAnnouncedAddress());
	...
	private static void sendToSomePeers(final JSONObject request) {
		Peers.sendingService.submit(() -> {
			...
			for (final Peer peer : Peers.peers.values()) {
				...
				if (Peers.enableHallmarkProtection && peer.getWeight() < Peers.pushThreshold) continue;
				...
				if (expectedResponses.size() >= Peers.sendToPeersLimit - successful) {
					for (final Future<JSONObject> future : expectedResponses)
                        try {
                            ...
                            if (response != null && response.get("error") == null) successful += 1;
                        } catch (final InterruptedException e) {
                            Thread.currentThread().interrupt();
                        } catch (final ExecutionException e) {
				...
				if (successful >= Peers.sendToPeersLimit) return;
			}
		});
	}
	...
	private static void sendToSomeSnPeers(final JSONObject request) {
		Peers.sendingService.submit(() -> {
			...
			for (final Peer peer : Peers.getConnectedSnPeers()) {
				...
				if (successful >= Peers.sendToSnPeersLimit) return;
			}
		});
	}
	...
	static void setAnnouncedAddress(final PeerImpl peer, final String newAnnouncedAddress) {
		...
		if (oldPeer != null) {
			...
			if (oldAnnouncedAddress != null && !Objects.equals(oldAnnouncedAddress, newAnnouncedAddress)) {
				Logger.logDebugMessage(
		...
		if (newAnnouncedAddress != null) {
			...
			if (oldHost != null && !Objects.equals(peer.getHost(), oldHost)) {
				...
				if (oldPeer != null) Peers.notifyListeners(oldPeer, Event.REMOVE);
			}
		}
	...
	public static boolean setCommunicationLoggingMask(final String[] events) {
		...
		int mask = 0;
		if (events != null) for (final String event : events) {
            switch (event) {
                case "EXCEPTION":
		...
		if (updated) Peers.communicationLoggingMask = mask;
		return updated;
	}

	public static void shutdown() {
		if (Init.peerServer != null) try {
