...
public final class Account {
	...
	static {
		...
		Nxt.getBlockchainProcessor().addListener(block -> {
			...
			try (DbIterator<AccountLease> leases = Account.getLeaseChangingAccounts(height)) {
				while (leases.hasNext()) changingLeases.add(leases.next());
			}
			...
			try (DbIterator<AccountSupernodeDeposit> deposits = Account.getRelevantSupernodeDepositEvents(height)) {
				while (deposits.hasNext()) changingDeposits.add(deposits.next());
			}
				...
				if (height == deposit.currentDepositHeightFrom) {
					...
					if(!participantAccount.isGuardNode())
						if (depositAccount.getUnconfirmedBalanceNQT() < Constants.SUPERNODE_DEPOSIT_AMOUNT) {
							...
							final LedgerEvent event = LedgerEvent.SUPERNODE_DEPOSIT;
							participantAccount.addToBalanceAndUnconfirmedBalanceNQT(event, lessor.getId(),
									Constants.SUPERNODE_DEPOSIT_AMOUNT);
	...
	private Account(final ResultSet rs, final DbKey dbKey) throws SQLException {
		...
		else this.controls = Collections.emptySet();
	}
	...
	private static Account newAccount(DbKey dbKey){
	...
	public static Account addOrGetAccount(final long id) {
		if (id == 0) throw new IllegalArgumentException("Invalid accountId 0");
	...
	private static void checkBalance(final long accountId, final long confirmed, final long unconfirmed) {
		if (accountId == Genesis.CREATOR_ID) return;
		if (confirmed < 0)
	...
	public static byte[] decryptFrom(final byte[] publicKey, final EncryptedData encryptedData,
		...
		if (uncompress && (decrypted.length > 0)) decrypted = Convert.uncompress(decrypted);
		return decrypted;
	}

	public static EncryptedData encryptTo(final byte[] publicKey, byte[] data, final String senderSecretPhrase,
			final boolean compress) {
		if (compress && (data.length > 0)) data = Convert.compress(data);
	...
	public static Account getAccount(final byte[] publicKey) {
		...
		if ((account.publicKey == null) || (account.publicKey.publicKey == null)
				|| Arrays.equals(account.publicKey.publicKey, publicKey)) return account;
	...
	public static byte[] getPublicKey(final long id) {
		...
		byte[] key = null;
		if (Account.publicKeyCache != null) key = Account.publicKeyCache.get(dbKey);
		if (key == null) {
			final PublicKey publicKey = Account.publicKeyTable.get(dbKey);
			if ((publicKey == null) || ((key = publicKey.publicKey) == null)) return null;
			if (Account.publicKeyCache != null) Account.publicKeyCache.put(dbKey, key);
		}
		return key;
	}
	...
	static boolean setOrVerify(final long accountId, final byte[] key) {
		...
		PublicKey publicKey = Account.publicKeyTable.get(dbKey);
		if (publicKey == null) publicKey = Account.publicKeyTable.newEntity(dbKey);
	...
	public boolean isGuardNode() {
		...
		guardNode = Arrays.stream(Constants.GUARD_NODES).anyMatch(g -> g == myId);
	...
	private void addToBalanceAndUnconfirmedBalanceNQT(final LedgerEvent event, final long eventId, final long amountNQT,
													  final long feeNQT) {
		if ((amountNQT == 0) && (feeNQT == 0)) return;
		...
		if (AccountLedger.mustLogEntry(this.id, true)) {
			if (feeNQT != 0) AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
	...
	private void addToGuaranteedBalanceNQT(final long amountNQT) {
		...
		try (Connection con = Db.db.getConnection();
			...
			try (ResultSet rs = pstmtSelect.executeQuery()) {
				...
				if (rs.next()) additions = Math.addExact(additions, rs.getLong("additions"));
	...
	void apply(final byte[] key) {
		...
		if (publicKey == null) publicKey = Account.publicKeyTable.newEntity(this.dbKey);
		if (publicKey.publicKey == null) {
			publicKey.publicKey = key;
			Account.publicKeyTable.insert(publicKey);
		} else if (!Arrays.equals(publicKey.publicKey, key)) throw new IllegalStateException("Public key mismatch");
			...
			if ((dbPublicKey == null) || (dbPublicKey.publicKey == null)) Account.publicKeyTable.insert(publicKey);
		}
		if (Account.publicKeyCache != null) Account.publicKeyCache.put(this.dbKey, key);
	...
	public byte[] decryptFrom(final EncryptedData encryptedData, final String recipientSecretPhrase,
		...
		if (key == null) throw new IllegalArgumentException("Sender account doesn't have a public key set");
	...
	public EncryptedData encryptTo(final byte[] data, final String senderSecretPhrase, final boolean compress) {
		final byte[] key = Account.getPublicKey(this.id);
		if (key == null) throw new IllegalArgumentException("Recipient account doesn't have a public key set");
	...
	private long getPseudoEffectiveBalanceNXT(final int height) {
		...
		try {
			long effectiveBalanceNQT = this.getLessorsGuaranteedBalanceNQT(height);
			if (this.activeLesseeId == 0) effectiveBalanceNQT += this.getBalanceNQT();
			return (effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0
					: effectiveBalanceNQT / Constants.ONE_NXT;
		} finally {
			Nxt.getBlockchain().readUnlock();
		...
		try {
			...
			try (Connection con = Db.db.getConnection();
				...
				try (ResultSet rs = pstmt.executeQuery()) {
					if (!rs.next()) return this.balanceNQT;
	...
	private long getLessorsGuaranteedBalanceNQT(final int height) {
		...
		try (DbIterator<Account> iterator = this.getLessors(height)) {
			while (iterator.hasNext()) lessors.add(iterator.next());
		}
		...
		try (Connection con = Db.db.getConnection();
			...
			//noinspection SuspiciousNameCombination
			if (height < blockchainHeight) pstmt.setInt(3, height);
			long total = 0;
			int i = 0;
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					final long accountId = rs.getLong("account_id");
					while ((lessorIds[i] < accountId) && (i < lessorIds.length)) total += balances[i++];
					if (lessorIds[i] == accountId) total += Math.max(balances[i++] - rs.getLong("additions"), 0);
				}
			}
			while (i < balances.length) total += balances[i++];
			return total;
		} catch (final SQLException e) {
			throw new RuntimeException(e.toString(), e);
		}
	...
	void leaseEffectiveBalance(final long lesseeId, final int period) {
		...
		if (accountLease == null) accountLease = new AccountLease(this.id, height + Constants.LEASING_DELAY,
	...
	private Pair<Integer, Integer> getSupernodeTimeframe(){
		if(!this.isSuperNode()) return new Pair<>(0, 0);
		else{
            AccountSupernodeDeposit deposit = Account.accountSupernodeDepositTable.get(Account.accountDbKeyFactory.newKey(this));
            if (deposit == null) return new Pair<>(0, 0);
			else return new Pair<>(deposit.currentDepositHeightFrom, deposit.currentDepositHeightTo);
        }
	}

	public void invalidateSupernodeDeposit() {
		if(!this.isSuperNode() || this.isGuardNode()) return;
	...
	boolean canBecomeSupernode(){
		return this.getUnconfirmedBalanceNQT() >= Constants.SUPERNODE_DEPOSIT_AMOUNT;
	}
	...
	void refreshSupernodeDeposit(String[] uris) throws IOException {
		...
		if(!this.isSuperNode() && this.getUnconfirmedBalanceNQT()<Constants.SUPERNODE_DEPOSIT_AMOUNT) return;
	...
	private boolean isRelevant(){
		...
		relevant = Arrays.stream(Constants.GUARD_NODES).anyMatch(sn -> thisid == sn);
	...
	private void save() {
		if ((!isRelevant()) && ((this.balanceNQT == 0) && (this.unconfirmedBalanceNQT == 0) && (this.forgedBalanceNQT == 0)
				&& (this.activeLesseeId == 0) && (!this.supernodeDepositBlocked) && this.controls.isEmpty()))
			Account.accountTable.delete(this, true);
		else Account.accountTable.insert(this);
	}
	...
	void setAccountInfo(String name, String description) {
		...
		if (accountInfo == null) accountInfo = new AccountInfo(this.id, name, description);
		else {
	...
	public static final class AccountInfo {
		...
		private void save() {
			if ((this.name != null) || (this.description != null)) Account.accountInfoTable.insert(this);
			else Account.accountInfoTable.delete(this);
		}
	...
	public static final class AccountSupernodeDeposit {
		...
		private AccountSupernodeDeposit(final ResultSet rs, final DbKey dbKey) throws SQLException {
			...
			this.uris.addAll(Arrays.asList(a.split(",")));
