...
final class VerifyTraceFile {
	...
	private static final List<String> assetQuantityHeaders = Arrays.asList("asset balance",
	...
	private static final List<String> currencyBalanceHeaders = Arrays.asList("currency balance",
			"unconfirmed currency balance");
	...
	public static void main(final String[] args) {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
			...
			while (true) {
				...
				for (int i = 0; i < headers.length; i++) valueMap.put(headers[i], values[i]);
				...
				if (Objects.equals("asset issuance", event)) {
					final String assetId = valueMap.get("asset");
					issuedAssetQuantities.put(assetId, Long.parseLong(valueMap.get("asset quantity")));
				}
				if (Objects.equals("asset transfer", event) && (Genesis.CREATOR_ID == Convert.parseUnsignedLong(accountId))) {
					final String assetId = valueMap.get("asset");
				...
				if (Objects.equals("asset delete", event)) {
					final String assetId = valueMap.get("asset");
					final long deletedQuantity = -Long.parseLong(valueMap.get("asset quantity"));
					final long currentQuantity = issuedAssetQuantities.get(assetId);
					issuedAssetQuantities.put(assetId, currentQuantity - deletedQuantity);
				...
				if (Objects.equals("currency issuance", event)) {
				...
				if (Objects.equals("crowdfunding", event)) {
					final String currencyId = valueMap.get("currency");
					issuedCurrencyUnits.put(currencyId, Long.parseLong(valueMap.get("crowdfunding")));
				}
				if (Objects.equals("currency mint", event)) {
					final String currencyId = valueMap.get("currency");
					issuedCurrencyUnits.put(currencyId,
							Math.addExact(VerifyTraceFile.nullToZero(issuedCurrencyUnits.get(currencyId)),
									Long.parseLong(valueMap.get("currency units"))));
				}
				if (Objects.equals("currency claim", event)) {
					final String currencyId = valueMap.get("currency");
					issuedCurrencyUnits.put(currencyId,
							Math.addExact(VerifyTraceFile.nullToZero(issuedCurrencyUnits.get(currencyId)),
									Long.parseLong(valueMap.get("currency units"))));
				...
				for (final Map.Entry<String, String> mapEntry : valueMap.entrySet()) {
					...
					if (VerifyTraceFile.isBalance(header)) accountTotals.put(header, Long.parseLong(value));
			...
			for (final Map.Entry<String, Map<String, Long>> mapEntry : totals.entrySet()) {
				...
				for (final String balanceHeader : VerifyTraceFile.balanceHeaders)
                    System.out.println(
                            balanceHeader + ": " + VerifyTraceFile.nullToZero(accountValues.get(balanceHeader)));
				...
				for (final Map.Entry<String, Map<String, Long>> assetMapEntry : accountAssetMap.entrySet()) {
					...
					if (Objects.equals(assetId, fxtAssetId)) continue;
					...
					if ((assetBalance != totalAssetDelta)
							&& ((Genesis.CREATOR_ID != Convert.parseUnsignedLong(accountId)) || (assetBalance != 0))) {
				...
				for (final Map.Entry<String, Map<String, Long>> currencyMapEntry : accountCurrencyMap.entrySet()) {
					...
					final long previousCurrencyQuantity = VerifyTraceFile
							.nullToZero(accountCurrencyUnits.get(currencyId));
			...
			for (final Map.Entry<String, Long> assetEntry : issuedAssetQuantities.entrySet()) {
				...
				if (issuedAssetQuantity != VerifyTraceFile.nullToZero(accountAssetQuantities.get(assetId))) {
					System.out
							.println("ERROR: asset " + assetId + " balances don't match, issued: " + issuedAssetQuantity
			...
			for (final Map.Entry<String, Long> currencyEntry : issuedCurrencyUnits.entrySet()) {
				...
				if (issuedCurrencyQuantity != VerifyTraceFile.nullToZero(accountCurrencyUnits.get(currencyId))) {
					System.out.println(
							"ERROR: currency " + currencyId + " balances don't match, issued: " + issuedCurrencyQuantity
			...
			} else System.out.println("SUCCESS: all " + totals.size()
			...
			} else System.out.println("SUCCESS: all " + issuedCurrencyUnits.size() + " currency units are correct!");
