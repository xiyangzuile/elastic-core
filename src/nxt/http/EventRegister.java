...
public class EventRegister extends APIServlet.APIRequestHandler {
	...
	static {
		EventRegister.eventsRegistered.put("registered", true);
	}

	/** Mutually exclusive parameters */
	private static final JSONObject exclusiveParams = new JSONObject();
	static {
		EventRegister.exclusiveParams.put("errorCode", 4);
		EventRegister.exclusiveParams.put("errorDescription", "Mutually exclusive 'add' and 'remove'");
	}

	/** Incorrect event */
	private static final JSONObject incorrectEvent = new JSONObject();
	static {
		EventRegister.incorrectEvent.put("errorCode", 4);
		EventRegister.incorrectEvent.put("errorDescription", "Incorrect event name format");
	}

	/** Unknown event */
	private static final JSONObject unknownEvent = new JSONObject();
	static {
		EventRegister.unknownEvent.put("errorCode", 5);
		EventRegister.unknownEvent.put("errorDescription", "Unknown event name");
	}

	/** No events registered */
	private static final JSONObject noEventsRegistered = new JSONObject();
	static {
		EventRegister.noEventsRegistered.put("errorCode", 8);
		EventRegister.noEventsRegistered.put("errorDescription", "No events registered");
	}
	...
	protected JSONStreamAware processRequest(final HttpServletRequest req) {
		...
		//
		final List<EventRegistration> events = new ArrayList<>();
		final String[] params = req.getParameterValues("event");
		if (params == null) {
			//
